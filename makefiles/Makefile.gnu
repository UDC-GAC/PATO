...
patch_compilationopts:
	@sed -i "s/seqan::setCompilationOpts(parser,.*);/`echo "seqan::setCompilationOpts(parser, \"CXX=${CXX} CXXFLAGS=${CXXFLAGS} LD=${LD} LDFLAGS=${LDFLAGS}\");" | sed -e 's/  */ /g'`/g" src/command_line_parser.hpp
